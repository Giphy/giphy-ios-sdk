...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `GiphyYYAnimatedImage` protocol,
 ...
 Sample Code:
     ...
     // ani@3x.gif
     GiphyYYImage *image = [GiphyYYImage imageNamed:@"ani"];
     GiphyYYAnimatedImageView *imageView = [GiphyYYAnimatedImageView alloc] initWithImage:image];
     [view addSubView:imageView];
 */
@interface GiphyYYAnimatedImageView : UIImageView
...
/**
 The GiphyYYAnimatedImage protocol declares the required methods for animated image
 display with GiphyYYAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to GiphyYYAnimatedImageView.image or GiphyYYAnimatedImageView.highlightedImage
 to display animation.
 
 See `GiphyYYImage` and `GiphyYYFrameImage` for example.
 */
@protocol GiphyYYAnimatedImage <NSObject>
@required
